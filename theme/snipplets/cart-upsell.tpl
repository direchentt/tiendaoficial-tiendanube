...
<div class="cart-upsell-module" style="background: rgba(98, 108, 126, 0.85); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid rgba(255, 255, 255, 0.4); color: #fff; padding: {{ pad }}; border-radius: 16px; margin: 0 15px 20px 15px; box-shadow: 0 8px 32px rgba(0,0,0,0.15);">
    <h5 style="text-align: center; font-size: 0.95rem; margin-bottom: 15px; font-weight: 400; color: rgba(255, 255, 255, 0.9); letter-spacing: normal;">
    ...
    <div class="cart-upsell-scroll" style="display: flex; overflow-x: auto; scroll-snap-type: x mandatory; gap: 12px; padding-bottom: 5px; scrollbar-width: none;">
        {% for p in cart_upsells|take(6) %}
            <div class="cart-upsell-item" style="flex: 0 0 {% if is_pdp %}95%{% else %}90%{% endif %}; scroll-snap-align: start; display: flex; align-items: center; justify-content: space-between; background: transparent; padding: 12px; border-radius: 20px; border: 1px solid rgba(255, 255, 255, 0.4);">
                <div style="display: flex; align-items: center; gap: 14px;">
                    ...
                    <div style="display: flex; flex-direction: column; line-height: 1.4;">
                        <a href="{{ p.url }}" title="{{ p.name }}" style="color: #fff; font-size: {{ title_size }}; font-weight: 400; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; text-transform: none;">
                            {{ p.name }}
                        </a>
                        <span style="font-size: {{ price_size }}; color: rgba(255,255,255,0.8); margin-top: 4px; font-weight: 400; display: inline-block;">
                            {% if p.display_price %}
                                {{ p.price | money }}
                            {% endif %}
                        </span>
                ...
                <form method="post" action="{{ store.cart_url }}" class="js-product-form" style="margin: 0; flex-shrink: 0;" data-store="product-form-{{ p.id }}">
                    ...
                    {% endif %}
                    <button type="submit" class="js-addtocart js-cart-upsell-btn" aria-label="Agregar" style="background: #fff; color: #000; border: 1px solid rgba(0,0,0,0.1); padding: 9px 18px; font-size: 0.85rem; font-weight: 500; border-radius: 30px; cursor: pointer; transition: all 0.2s;">
                        Agregar
                    </button>
