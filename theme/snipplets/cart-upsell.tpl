...
<div class="cart-upsell-module" style="background: rgba(15, 20, 25, 0.75); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid rgba(255, 255, 255, 0.15); border-top: 1px solid rgba(255, 255, 255, 0.25); color: #fff; padding: {{ pad }}; border-radius: 20px; margin: 0 15px 20px 15px; box-shadow: 0 8px 32px rgba(0,0,0,0.3);">
    <h5 style="text-align: center; font-size: 0.9rem; margin-bottom: 15px; font-weight: 600; color: #fff; letter-spacing: 0.02em;">
    ...
    <div class="cart-upsell-scroll" style="display: flex; overflow-x: auto; scroll-snap-type: x mandatory; gap: 12px; padding-bottom: 5px; scrollbar-width: none;">
        {% for p in cart_upsells|take(6) %}
            <div class="cart-upsell-item" style="flex: 0 0 {% if is_pdp %}95%{% else %}90%{% endif %}; scroll-snap-align: start; display: flex; align-items: center; justify-content: space-between; background: rgba(255, 255, 255, 0.06); padding: 10px 12px; border-radius: 16px; border: 1px solid rgba(255, 255, 255, 0.1);">
                <div style="display: flex; align-items: center; gap: 12px;">
                    ...
                    <div style="display: flex; flex-direction: column; line-height: 1.3;">
                        <a href="{{ p.url }}" title="{{ p.name }}" style="color: #fff; font-size: {{ title_size }}; font-weight: 600; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">
                            {{ p.name }}
                        </a>
                        <span style="font-size: {{ price_size }}; color: rgba(255,255,255,0.7); margin-top: 4px; font-weight: 500; display: inline-block; padding-top: 2px;">
                            {% if p.display_price %}
                                {{ p.price | money }}
                            {% endif %}
                        </span>
                ...
                <form method="post" action="{{ store.cart_url }}" class="js-product-form" style="margin: 0; flex-shrink: 0;" data-store="product-form-{{ p.id }}">
                    ...
                    {% endif %}
                    <button type="submit" class="js-addtocart js-cart-upsell-btn" aria-label="Agregar" style="background: rgba(255,255,255,0.9); color: #111; border: none; padding: {{ btn_pad }}; font-size: 0.8rem; font-weight: 600; border-radius: 20px; cursor: pointer; transition: all 0.2s; box-shadow: 0 2px 5px rgba(0,0,0,0.2);">
                        Agregar
                    </button>
